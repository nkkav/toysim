...
void toy_syscall::get_buffer(int argn, unsigned char* buf, unsigned int size)
{
  unsigned int addr = RB[2+argn];
...
void toy_syscall::set_buffer(int argn, unsigned char* buf, unsigned int size)
{
  unsigned int addr = RB[2+argn];
...
void toy_syscall::set_buffer_noinvert(int argn, unsigned char* buf, unsigned int size)
{
  unsigned int addr = RB[2+argn];

//  for (unsigned int i = 0; i<size; i+=4, addr+=4) {
  for (unsigned int i = 0; i<size; i+=1, addr+=1) {
    DM.write_byte(addr, *(unsigned int *) &buf[i]);
  }
}

int toy_syscall::get_int(int argn)
{
  return RB[2+argn];
}

void toy_syscall::set_int(int argn, int val)
{
  RB[2+argn] = val;
}

void toy_syscall::return_from_syscall()
{
  ac_pc = RB[15];
//  npc = ac_pc + 1;
}

void toy_syscall::set_prog_args(int argc, char **argv)
{
  int i, j, base;
  ...
  RB[13] = AC_RAM_END-512;
  set_buffer(12, (unsigned char*) ac_argstr, 512);   //$25 = $29(sp) - 4 (set_buffer adds 4)

  //Ajust %sp and write string pointers
  RB[13] = AC_RAM_END-512-120;
  set_buffer_noinvert(25, (unsigned char*) ac_argv, 120);

  //Set %sp
  RB[13] = AC_RAM_END-512-128;

  //Set %o0 to the argument count
  RB[2] = argc;

  //Set %o1 to the string pointers
  RB[3] = AC_RAM_END-512-120;
}
